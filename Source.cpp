		...
		while (!isTheEndOfProgram) {
			...
			//isTheEndOfPlay = false;
				...
				for (Client* client : server.clients) {
					...
					player.playerBody.setFillColor(color);
				...
				for (int i = 0; i < enemyAmount; i++) {
					...
					enemiesPacket << enemyArr[i].getPlayerSize();

					colorTemp = enemyArr[i].enemyBody.getFillColor();
					enemiesPacket << colorTemp.r;
					enemiesPacket << colorTemp.g;
					enemiesPacket << colorTemp.b;
				}
				...
				for (int i = 0; i < foodAmount; i++) {
					...
					foodPacket << colorTemp.r;
					foodPacket << colorTemp.g;
					foodPacket << colorTemp.b;
				}

				server.SendPacketToAllClients(foodPacket);
				

			while (!is_all_players_died) {
